...
## Operations and CSV earnings preview — September 5
Added Recorded production beside Lead cohorts, using direct dated person_stage_log milestones independently of lead creation. Tenant/person/stage deduplication; seed, unknown team and invalid/future dates excluded. This is observed milestone history, NOT certified actual transaction dates or complete production. No mismatched intake denominator is used to make a conversion ratio.

Earnings at /earnings: browser-only CSV import, column mapping, strict amount/date checks, duplicate deal-side rejection, default brokerage share and per-row override, review/accept step, agent filtering and formula-safe statement export. Formula: (gross minus referral fee) times brokerage share, minus entered expenses. Missing expenses remain unknown and subtotal coverage is explicit. No files or financial values are sent to a server; leaving clears earnings. FUB commission units/coverage not yet certified, so no automatic GCI estimate from those fields.

Assignment evidence: optional CSV with agent IDs, lead IDs and timezone-qualified assignment dates. Rejects unknown agent IDs; distinct leads per agent in MTD trigger Review pause at the saved allowance. Evidence lists file/row/timestamp and Zillow link. Imported evidence is held in a React provider scoped by session user and org; refresh/sign-out/org switch clears it. Same imported review appears in Today. Import completeness is never certified. Automated FUB assignment ingestion and five-point conversion deterioration remain pending historical coverage.

Today: mark existing commitment done and undo with server response validation; demo is local only. Actual owner RLS writes not exercised. Coach-to-Rep carries one selected focus into a review-date/evidence-note draft, filters Rep to the agent, and appears in Today. Export available; actual agreed commitment must still be recorded in existing 1:1. No training assignment is sent. Calendar integration and durable practice due dates remain unfinished.

Validation: typecheck passes, 411 tests pass with existing Zillow slide hash failure. Browser checked recorded production, CSV review ($18k gross, $4.8k brokerage share, $1.9k known contribution with one missing-cost row), accept/export availability, demo commitment completion, Coach-to-Rep context handoff. No live customer modifications, production or backend deploy.
