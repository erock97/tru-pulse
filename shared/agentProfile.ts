...
export type PersonalProfile={bio:string;headline:string;location:string;markets:string;careerStart:string;goal:string;favorite:string;theme:typeof PROFILE_THEMES[number];font:'classic'|'modern'|'editorial';coverStyle:'orbit'|'waves'|'grid';accent:string;portrait:string;cover:string;interests:string[];gallery:{caption:string;image:string}[];sections:ProfileSection[];hidden:ProfileSection[]};
export type ProfileBadge={id:string;title:string;detail:string;verifiedAt:string;kind:'training'|'contract'};
export const DEFAULT_PROFILE:PersonalProfile={bio:'',headline:'',location:'',markets:'',careerStart:'',goal:'',favorite:'',theme:'sage',font:'classic',coverStyle:'orbit',accent:'#bdd1ed',portrait:'',cover:'',interests:[],gallery:[],sections:[...PROFILE_SECTIONS],hidden:[]};
...
export function validateProfile(raw:unknown):PersonalProfile {
 ...
 return {headline:text('headline',100),bio:text('bio',1200),location:text('location',80),markets:text('markets',160),goal:text('goal',280),favorite:text('favorite',160),careerStart:year,theme:one('theme',PROFILE_THEMES),font:one('font',['classic','modern','editorial']),coverStyle:one('coverStyle',['orbit','waves','grid']),accent,portrait:image(p.portrait),cover:image(p.cover),interests,gallery,sections:sections as ProfileSection[],hidden:hidden as ProfileSection[]};
}
