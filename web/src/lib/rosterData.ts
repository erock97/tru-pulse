...
export const WINDOWS: readonly Window[] = [
  ...
  { key: 'all', label: 'One year', days: 365 },
  { key: '6mo', label: '6mo', days: '6mo' },
  { key: '2yr', label: '2 years', days: '2yr' },
  { key: 'ytd', label: 'Year to date', days: 'ytd' },
];
export const PERIOD_OPTIONS = ['mtd','2yr','all','ytd','6mo','90d','14d','7d'].map(key=>{const w=WINDOWS.find(w=>w.key===key)!;return {value:w.key,label:w.label==='7d'?'7 days':w.label==='90d'?'90 days':w.label==='6mo'?'6 months':w.label}});
