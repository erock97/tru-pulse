...
const reference: Record<number, string> = {
  1: '<h3>Practice in TrueHQ</h3><p>Stay in this session. Review the People-list reference, then complete each simulated contact exercise here. No separate Follow Up Boss account is needed for class.</p><h3>Before leaving a contact</h3><p>Check the true stage and saved change, a factual note, and a dated next task. Another agent should understand what happened and what to do next.</p><p>After the first appointment: Met with customer. After the second appointment: Showing homes. Record an accepted offer under Under contract and add the deal.</p>',
  ...
  3: '<h3>After the showing</h3><p>Start with “Did we see any homes we want to write an offer on?” Learn what explains the rating and what is missing. Discuss financing preparation and the reason for timing. Agree on a useful next action, date, and owner.</p><p>Confirm current brokerage agreement requirements before touring. Save the true stage, specific preferences, timing, and a dated task.</p>',
  4: '<h3>ZHL overview and buyer introductions</h3><p>Use Section 1 to review loan options, affordability estimates, approval stages and loan-officer coordination. Section 2 covers the end-of-appointment introduction and breakout practice.</p><p>Confirm purchase timing, the existing lender and the question the buyer wants answered. For more than six months out with no request for financing help now, agree on agent follow-up. For a near-term buyer asking about credit options, let the loan officer assess eligibility. Ask permission before connecting anyone.</p><p>After an accepted introduction, confirm the call with both people and create a FUB follow-up task.</p>',
};
  ...
  useEffect(() => {
    ...
    fetch(`/workshops/day${day}.json`,{signal:controller.signal})
      ...
      .then(async data=>{
        ...
        root.querySelector('#resource-body')!.innerHTML=(day===1?reference[day]+data.resources:reference[day])+`<p><a href="/workshops/day${day}-resources.html" target="_blank" rel="noopener">Open complete agent reference and practice notes ↗</a></p><p><a href="/workshops/day${day}-review.html" target="_blank" rel="noopener">Open the slide deck for independent review ↗</a></p>`;
