...
//
// This form has an unusual property: it is evidence. Twilio's campaign review
// asks for a screenshot of it, and a carrier decides whether to approve the
// number based on what is visible here. Everything below is present because a
...
//   · a checkbox that starts UNCHECKED, separate from any other agreement
//
...
//   2. There is always a way past without agreeing. Consent obtained by blocking
//      someone from the product they need for their job is not consent, and a
//      reviewer who sees no way out will read it the same way.
//
// The consent sentence is NOT written here — it comes from shared/smsConsent.ts,
// and the Worker writes its own copy of that same constant into the ledger. That
// is what lets us say, later, that the words stored are the words shown.
//
// Rendered in exactly two places: the last step of account creation
// (SetPassword.tsx), which is where consent is actually collected, and a card on
// Agent HQ's Home tab, which is how someone changes their mind later. There is
// deliberately no third prompt — the campaign filed with the carrier states that
// consent is collected at account creation, and a stray extra ask would make
// that untrue.
...
/** What they are told they are signing up for, in their words rather than the
 *  carrier's. Sits next to the legal sentence, never instead of it. */
const EXAMPLES = [
...
export default function SmsConsent({
  sms, onSaved, onDeclined, declineLabel, ctaClass = 'ah-cta',
}: {
  sms: AgentSms;
  onSaved: () => void;
  /** Omit to hide the decline button. The Home card has nothing to decline —
   *  not filling it in IS declining. */
  onDeclined?: () => void;
  declineLabel?: string;
  /** The account-creation card and Agent HQ load different stylesheets, so the
   *  caller says which button class actually exists on its page. */
  ctaClass?: string;
}) {
  ...
  // Only complain once they have typed enough for it to be wrong, rather than
  // colouring the field red on the first keystroke.
  ...
  // ── Already on ─────────────────────────────────────────────────────────────
  // Someone who has consented gets one thing from this form: the ability to
  // stop. No re-confirmation and no "are you sure" — an opt-out is one click.
  if (sms.reachable) {
    return (
      <div className="sms">
        <p className="sms-on">
        ...
        <button className="sms-off" disabled={busy !== null} onClick={() => void optOut()}>
          {busy === 'out' ? 'Turning off…' : 'Turn off text messages'}
        </button>
        {err && <div className="sms-err">{err}</div>}
      </div>
    );
  }
  ...
  return (
    <div className="sms">
      ...
      {phoneLooksWrong && (
        <div className="sms-warn">That doesn’t look like a US mobile number.</div>
      )}
      {e164 && <div className="sms-ok">We’ll text {formatUS(e164)}.</div>}
      ...
      {/*
        ...
        agreement on the screen and do not default it to true — either would
      ...
      */}
      ...
      {onDeclined && (
        <button className="sms-skip" disabled={busy !== null} onClick={() => void decline()}>
