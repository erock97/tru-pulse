...
export function ProductionPanel({period,orgId}:{period:PulsePeriod;orgId?:string}) {
  ...
  useEffect(()=>{let active=true;loadDashboard(orgId).then(d=>{if(active)setData(d);}).catch(()=>{if(active)setError('Production history could not be loaded.');});return()=>{active=false;};},[orgId]);
  ...
  return <section className="production-panel"><h2>Recorded production</h2><p>Milestones dated in this period, including leads created earlier. Counts reflect available history; missing history is not zero production. First-observed dates may differ from the actual transaction date.</p>
    <div className="pulse-summary">{(['offer','uc','closed'] as const).map(k=><div key={k}><span>{k==='uc'?'Under-contract records':k==='closed'?'Closed records':'Offer records'}</span><strong>{data.stageLog?.length ? result.counts[k] : '—'}</strong><small>Dated milestone credits</small></div>)}</div>
    <p>{result.excluded} records lack a usable date or team identity, or are undated seeds. Observed milestones and cumulative credits are distinguished by date source. Skipped earlier stages use the qualifying later event date.</p>
