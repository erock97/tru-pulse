// TRU Rep — "The Record Is the Job" as ONE module (FUB operating system).
// Usage: node rep_record_is_the_job.mjs <path-to-secrets.json>
//
// WHAT THIS FILE IS. Kayla's locked AgentCourse screens — not a slide deck,
// not Day 1/2/3, not a July rename. Practice cards reuse PracticeRecord packs
// honest-stage and day-close. There is no Zillow track link.
//
...
export const MODULE_ID = 'a9999999-9999-9999-9999-999999999999';
export const TITLE = 'The Record Is the Job';
...
async function seed() {
  if (!process.argv[2]) throw new Error('Usage: node rep_record_is_the_job.mjs <path-to-secrets.json>');
  ...
  console.log(`module: 1, cards: ${CARDS.length}, questions: ${QUESTIONS.length} (no Zillow track link)`);
}
