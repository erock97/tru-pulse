...
--
-- Idempotent + additive. Safe to run more than once. Depends on schema.sql only
-- (the agents table and its auth_id link) — deliberately NOT on the
-- agent-experience migration, so this can ship without it.
--
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ── 4. What the browser asks for ─────────────────────────────────────────────
-- A single self-contained read, deliberately NOT folded into agent_home().
--
-- agent_home() belongs to the agent-experience work, which is still on an
-- unmerged branch. Hanging this feature off it would mean SMS consent could not
-- ship until that lands, and would silently break if it lands in a different
-- shape. This RPC depends on nothing but the columns above, so the consent flow
-- stands on its own.
--
-- Never returns the whole number. The card that renders this sits on a screen
-- that gets shown in team meetings; four digits is enough to recognise your own
-- phone, and the rest has no business travelling to a browser.
create or replace function agent_sms_state()
returns json language sql security definer set search_path = public as $$
  select case when a.id is null then null else json_build_object(
           'last_four',   right(coalesce(a.sms_phone, ''), 4),
           'has_phone',   a.sms_phone is not null,
           'consent_at',  a.sms_consent_at,
           'opt_out_at',  a.sms_opt_out_at,
           'prompted_at', a.sms_prompted_at,
           'reachable',   sms_reachable(a.sms_phone, a.sms_consent_at, a.sms_opt_out_at)
         ) end
    from (select * from agents where auth_id = auth.uid() limit 1) a;
$$;
revoke execute on function agent_sms_state() from public, anon;
grant  execute on function agent_sms_state() to authenticated;
