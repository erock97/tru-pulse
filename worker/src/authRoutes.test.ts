...
describe('acting as a team', () => {
  ...
  /** admins table says yes, the target already has an auth user, generate_link mints a
   *  hash, verify returns the team's session. `exists=false` models someone who was
   *  never invited (or never accepted) — generate_link would otherwise create them. */
