...
beforeAll(async()=>{
 ...
 await pg.exec(readFileSync(new URL('../../supabase/migrations/20260913153453_partial_report_release_policy.sql',import.meta.url),'utf8'));
...
describe('real SQL receipts and HTTP',()=>{
 ...
 it('explicit operator control releases supported coaching from a current partial report with coverage preserved and audited',async()=>{const f:any=fixture('partial',true);f.coverage.contacts=[{leadId:'missing-lead',leadName:'Missing Lead',agentName:'Example',status:'unresolved',reason:'collection_failed'}];f.agents=[{agentName:'Example',metrics:{},opportunities:[{explanation:'Supported coaching',patternKey:'supported-partial',findingIds:['partial-finding']}]}];f.findings=[{findingIndex:0,findingId:'partial-finding',agentName:'Example',leadName:'Observed Lead',occurredAt:'2030-01-03T12:00:00Z',quote:'Observed message'}];const accepted=await send('/coach/weekly-report',f);expect(accepted.status).toBe(201);const r=(await accepted.json() as any).receipt;const command=control(r,'release','release-partial');const result=await op(command);expect(result.status).toBe(200);expect(result.body.receipt.publicationStatus).toBe('published');expect(result.body.receipt.coverageState).toBe('partial');const stored=(await pg.query<any>("select payload from coach_weekly_reports where run_id='partial'")).rows[0].payload;expect(stored.coverage).toEqual(f.coverage);expect((await pg.query<any>("select count(*)::int n from coach_patterns where pattern_key='supported-partial'")).rows[0].n).toBe(1);const audit=(await pg.query<any>("select actor,reason from coach_report_audit where operation_id='release-partial'")).rows[0];expect(audit).toEqual({actor:'eric',reason:command.reason})});
 it('unknown coverage remains held even under explicit operator control',async()=>{const f:any=fixture('unknown-release');delete f.coverage;f.schemaVersion='1.0';expect((await send('/coach/weekly-report',f)).status).toBe(201);const r=await lookup('unknown-release');const result=await op(control(r,'release','release-unknown'));expect(result.status).toBe(409);expect(result.body.error).toBe('coverage_unknown');expect((await lookup('unknown-release')).publicationStatus).toBe('held')});
 ...
 it('old direct upsert/remapping cannot mutate a managed report',async()=>{await expect(pg.query("update coach_weekly_reports set status='published' where run_id='unknown-release'")).rejects.toThrow('explicit_control_required');await expect(pg.query("update coach_weekly_reports set team_id=$1 where run_id='partial'",[other])).rejects.toThrow('immutable_receipt')});
 it('same source evidence shared across reports survives withdrawal of one',async()=>{
  ...
  expect((await pg.query<any>("select count(*)::int n from coach_pattern_findings where finding_id='same-finding'")).rows[0].n).toBe(1);
  ...
  expect((await pg.query<any>("select count(*)::int n from coach_pattern_findings where finding_id='same-finding'")).rows[0].n).toBe(0);
 });
...
describe('lifecycle edge cases',()=>{
 ...
 it('explicit operator supersession can publish a partial replacement without changing coverage',async()=>{const a=await lookup('live-new'),b=await accept('partial-replacement',true);const result=await op(control(a,'supersede','partial-replace',{replacement:{runId:b.runId,expectedHash:b.payloadHash,expectedRevision:b.revision}}));expect(result.status).toBe(200);expect((await lookup(a.runId)).publicationStatus).toBe('superseded');expect((await lookup(b.runId)).publicationStatus).toBe('published');expect((await lookup(b.runId)).coverageState).toBe('partial')});
 it('unknown coverage cannot replace a published report',async()=>{const a=await lookup('partial-replacement');const f:any=fixture('unknown-replacement');delete f.coverage;f.schemaVersion='1.0';await send('/coach/weekly-report',f);const b=await lookup('unknown-replacement');const result=await op(control(a,'supersede','unknown-replace',{replacement:{runId:b.runId,expectedHash:b.payloadHash,expectedRevision:b.revision}}));expect(result.status).toBe(409);expect(result.body.error).toBe('coverage_unknown');expect((await lookup(a.runId)).publicationStatus).toBe('published');expect((await lookup(b.runId)).publicationStatus).toBe('held')});
...
describe('control recovery and transport',()=>{
 ...
 it('SQL keeps partial release denied unless the receiver supplies its policy grant',async()=>{const r=await accept('partial-policy-denied',true);const command=control(r,'release','partial-policy-denied');await expect(rpc('coach_receipt_control',{p_team:team,p_actor:'offline-operator',p_command:command,p_canonical:canonicalJson(command),p_allow_partial:false})).rejects.toThrow('partial_release_disabled');expect((await lookup(r.runId)).publicationStatus).toBe('held')});
...
describe('general partial-release policy',()=>{
 it('authorizes any valid team-scoped operator command without actor or run identity exceptions',async()=>{
  const flags:boolean[]=[];
  const db={rpc:async(_name:string,args:any)=>{flags.push(args.p_allow_partial);return {};}} as unknown as Db;
  const invoke=async(body:any)=>handleReportReceipts(new Request('https://offline.test/coach/weekly-report/control',{method:'POST',headers:{Authorization:'Bearer operator'},body:JSON.stringify(body)}),env,new URL('https://offline.test/coach/weekly-report/control'),{},db);
  for(const runId of ['current-signature-run','current-costigan-run']){
   await invoke({operationId:'release-'+runId,action:'release',teamId:team,runId,expectedHash:'a'.repeat(64),expectedRevision:1,reason:'Eric approval reference: current authorized run'});
   expect(flags.pop()).toBe(true);
  }
 });
});
