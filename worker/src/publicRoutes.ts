 ...
 *  toggle-commitment. They backed the old token-URL agent portal — anon-reachable
 *  reads and writes keyed on a UUID that travelled in a URL — and nothing in the
 *  app has called them since the cookie-auth agent surface replaced them. Their
 *  database grants are revoked in db/hq_agent_experience.sql; these entries go so
 *  the route 404s rather than reaching a function it can no longer execute.
 *
