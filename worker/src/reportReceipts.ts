...
export async function handleReportReceipts(req: Request, env: Env, url: URL, cors: Record<string, string>, database: Db): Promise<Response | null> {
  ...
  try {
    ...
    // Reaching control already proves an authenticated operator scoped to this team.
    // SQL still requires an explicit immutable command and audits actor, reason and state.
    ...
    const code = ['identity_conflict','payload_conflict','legacy_conflict','operation_conflict','revision_conflict','invalid_transition','partial_release_disabled','coverage_unknown','evidence_conflict'].find(c => message.includes(c));
