...
export async function handleReportReceipts(req: Request, env: Env, url: URL, cors: Record<string, string>, database: Db): Promise<Response | null> {
  ...
  try {
    ...
    const validRef = (v: any) => v && typeof v.runId === 'string' && ID.test(v.runId) && typeof v.expectedHash === 'string' && /^[a-f0-9]{64}$/.test(v.expectedHash) && Number.isSafeInteger(v.expectedRevision) && v.expectedRevision > 0;
